<template>
<div>
  <el-form label-position="left"  ref="simple_picture" :model="simple_picture" label-width="100px":style="{marginTop:'100px'}" style="margin:30px;width:80%;min-width:600px;" border>
    ...
    export default {
        ...
        methods: {
            ...
            handleChangeDisplay(value) {
                ...
                if (this.simple_picture.url) {
                    currentData.html = currentData.html.replace("DEMO", this.simple_picture.img).replace("HREF", this.simple_picture.url.trim()||"javascript:void(0)").replace("TEXT", this.simple_picture.text||"").replace(/MARGIN/g, this.simple_picture.margin);
                }
        ...
        mounted() {
            if (this.designCurrentData && this.designCurrentData.data) {
                ...
                if (!this.simple_picture.font_color) {
                    this.simple_picture.font_color = "#FFFFFF";
                }
                this.fileList = [{
                    name:"",
                    url: this.simple_picture.img || ""
